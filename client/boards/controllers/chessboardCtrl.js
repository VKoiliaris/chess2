angular.module('chess2').controller('ChessboardCtrl', ['$scope', '$meteor',
	function($scope, $meteor){
        ...
        $scope.kingIsChecked = false;
        $scope.startingBoard = {board:[
			...
			], castlingStatus: [true, true, true, true], first: true, white: '', black: ''};
        ...
        $scope.piecesImages = {
	  	...
	  	// --- View Methods ---
	  	$scope.setInitialBoard = function(currentUser){
	  		$scope.initialRequest = _.find($scope.boards, function(board){ return board.first === true; });
	  		if ($scope.initialRequest){
		  		if ((currentUser === $scope.initialRequest.white) || (currentUser === $scope.initialRequest.black)){
			  		$scope.initialRequest.board = _.map($scope.startingBoard.board, _.clone);
			  		$scope.initialRequest.castlingStatus = _.clone($scope.startingBoard.castlingStatus);
			  		$scope.whiteSidePlayer = $scope.initialRequest.white;
			  		$scope.blackSidePlayer = $scope.initialRequest.black;
			  		$scope.currentBoard = $scope.initialRequest.board;
			  		$scope.castling = $scope.initialRequest.castlingStatus;
			  		$scope.appReady = true;
			  	}
		  	}else {
		  		window.alert("No boards found!");
		  	}
	  	};

	  	$scope.boardToView = function(row, column){
	  		if ($scope.appReady === true){
	  			return $scope.piecesImages[_.last($scope.boards).board[row][column]];
	  		}
	  	};
	  	...
	  	$scope.whiteTurn = function(user, pieceSide){
       		return ((((($scope.boards.length-1) % 2) === 0) && (pieceSide === 'w') && (user === $scope.whiteSidePlayer)) || (((($scope.boards.length-1) % 2) !== 0) && (pieceSide === 'b') && (user === $scope.blackSidePlayer)));
       	};

		$scope.interactWithBoard = function(user, row, column){
				$scope.setBoard();
				...
				$scope.moveFromTo.push({square:$scope.currentBoard[row][column], row: row, column: column });
		  		...
		  		if ($scope.moveFromTo.length === 2){
		  			...
		  			if($scope.whiteTurn(user, $scope.moveFromTo[0].square[0])){
			  			...
			  			if (($scope.move($scope.moveFromTo[0].square, $scope.moveFromTo[0].row,  $scope.moveFromTo[0].column, $scope.moveFromTo[1].square, $scope.moveFromTo[1].row,  $scope.moveFromTo[1].column, $scope.currentBoard))
			  				&& (!$scope.kThreatened($scope.moveFromTo[0].square, $scope.newMove))) {
			  				...
			  				$scope.boards.push({board:$scope.newMove, castlingStatus:$scope.cstl, white:$scope.whiteSidePlayer, black:$scope.blackSidePlayer});
			  			}
		  			}
		  			$scope.setBoard();
		  			$scope.kThreatened($scope.moveFromTo[0].square, $scope.currentBoard, $scope.currentBoard);
		  			$scope.moveFromTo = [];
		  			$scope.chosen = false;
		  			return;
		  		}
		...
		// --- King moves ---
	  	...
	  	// --- Check for legal move of pieces the move in lines or return the line from the piece to the threatened piece ---
	    ...
	    // --- check for legal move of "special" moves ---
		$scope.specialMoves = {
		...
		$scope.checkMate = function (squareFrom, board, kingRow, kingColumn, enemyThreats){
			...
			for (var i = -1; i < 2; i++){
				for (var j = -1; j < 2; j++){
					if((kingRow+i > -1) && (kingRow+i < 8) && (kingColumn+i > -1) && (kingColumn+j < 8)){
			...
			for (var i = 0; i < enemyThreats.length; i++){
				...
				if (($scope.threateningEnemyInfo.piece[1] !== 'n') && ($scope.threateningEnemyInfo.piece[1] !== 'p')){
